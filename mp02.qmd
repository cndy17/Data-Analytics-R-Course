...
up_and_coming <- TITLE_PRINCIPALS |>
  ...
  left_join(TITLE_RATINGS, by = "tconst") |>
  group_by(primaryName) |>
  mutate(debut_year = min(startYear, na.rm = TRUE)) |>  # Get the minimum startYear for first movie they were a part of 
  filter(success_score >= 0.6, debut_year >= 2015) |>
  summarise(
    success_count = n(),
    mean_success_score = mean(success_score, na.rm = TRUE)) |>  # Ensure NA values are ignored
...
Getting voice actors, my guess is this is due to episodes and tv series having a higher count. It may help to filter out the TV episodes.
```{r echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Display Code" 
TITLE_BASICS |>
...
What about actors that do well in the drama genre?
```{r echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Display Code" 
actor_director_names |> 
  ...
  head(10)
```
Peter Jackson is the director of the Hobit. We can see how franchises and a series may skew things.
