...
Ryan Reynolds, Matt Damon, Tom Holland, Timothee Chalamet, Dave Bautistqa... I don't think I could consider them up and coming actors. Let us adjust adjust again. It seems like the startYear lowers the success count. It only counts for recent movies and so established actors show up too because their earlier successes are not counted. Does up and coming mean young or just that their first movie was recent? I think older personell can be up and coming if they are just coming into the scene. Let's try looking at the minimum start year of their first titles
...
TITLE_PRINCIPALS |>
  ...
  filter(debut_year >= 2015, success_count >= 10 & success_count <= 15) |>
  arrange(desc(mean_success_score), desc(success_count)) |>
  head(10) |>
  gt() |>
  tab_header(
...
```
Selecting Barry Keoghan from this list of up and coming actors, we can see some of his past titles. I would consider him as a potential actor for this film. 

## Combinations of Personel
...
What if we look at a specific director to see what actors and actresses they work well with. For example, if I wanted Greta Gerwig. Who has she worked well with in the past? 
...
actor_director_names |> 
  filter(! titleType %in% c("tvEpisode", "videoGame", "video")) |> # filter out tvEpisodes so results are not skewed by number of episodes
  filter(director_name == "Greta Gerwig") |>
  group_by(actor_name, director_name) |> # group by actor_name and director_name combinations
  summarise(
    success_count = n(), # count the number of times they've worked together
    mean_success_score = mean(success_score, na.rm = TRUE)) |> # find the mean success score
...
The classic movie I propose to remake is Princess Bride, directed by Greta Gerwig. 
```{r echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Display Code" 
TITLE_BASICS |>
...
The original has a large number of IMDb ratings, a high average rating, and has not  been remade in the past 25 years. A Princess Bride Home Movie was created during COVID, a miniseries shot on phones from home, but a full film remake would be very successful. 
...
Strong Box Office Appeal: By casting Emma Stone and Barry Keoghan, we leverage their proven track records; both actors have consistently delivered hits, with Emma starring in over 25 successful films and Barry Keoghan emerging as an up and coming actor with growing fans and success.
