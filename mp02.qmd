...
## Data
To do so, this project uses data made available by [Internet Movie Database (IMDb)](https://datasets.imdbws.com/). 
...
When working with NAME_BASICS, I can only have alive personnel on my team. 
```{r echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Display Code"
#| cache: true
...
## Up and Coming 
# lets say titles startyear 2010
# actors and actresses who have successful movies
# 5-10 Successful Titles Since 2010
...
Ryan Reynolds, Matt Damon, Tom Holland, Timothee Chalamet, Dave Bautistqa... I don't think I could consider them up and coming actors. Let us adjust adjust again. It seems like the startYear lowers the success count. It only counts for recent movies and so established actors show up too because their earlier successes are not counted. Does up and coming mean young or just that their first movie was recent? I think older personell can be up and coming if they are just coming into the scene. Let's try looking at the minimum start year of their movies.

```{r echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Display Code" 
#| eval: false
up_and_coming <- TITLE_PRINCIPALS |>
  filter(category == "actor") |>
  select(tconst, nconst) |>
  right_join(NAME_BASICS, by = "nconst") |>
  left_join(TITLE_BASICS, by = "tconst") |>
  left_join(TITLE_RATINGS, by = "tconst") |>
  group_by(primaryName) |>
  filter(success_score >= 0.6, min(startYear, na.rm = TRUE) >= 2015) |> # Get the minimum startYear for first movie they were a part of 
  summarise(
    success_count = n(),
    mean_success_score = mean(success_score, na.rm = TRUE)) |>  # Ensure NA values are ignored
  filter(success_count >= 1 & success_count <= 15) |>
  arrange(desc(success_count), desc(mean_success_score)) |>
  head(10)
...
up_coming <- TITLE_PRINCIPALS |>
  ...
  group_by(nconst) |>
  mutate(success_count = n()) |>
  mutate(mean_score = mean(success_score)) |>
  ungroup() |>
  filter(success_count >=1 & success_count <=5) |>
  arrange(desc(success_count), desc(mean_score)) |>
  select(primaryName,success_count, mean_score) 
```
...
I am getting mostly voice actors in the result above. My guess is this is due to episodes and tv series having a higher count. It may help to filter out the TV episodes.
```{r echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Display Code" 
TITLE_BASICS |>
...
### Top 10 Actor/Actress and Director Pairs
```{r echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Display Code" 
actor_director_names |> 
  filter(! titleType %in% c("tvEpisode", "videoGame", "video")) |> # filter out tvEpisodes so results are not skewed by number of episodes
  group_by(actor_name, director_name) |> # group by actor_name and director_name combinations
  summarise(
    success_count = n(), # count the number of times they've worked together
    mean_success_score = mean(success_score, na.rm = TRUE)) |> # find the mean success score
...
What about pairs that do well in the drama genre?
```{r echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Display Code" 
actor_director_names |> 
  ...
  summarise(
    success_count = n(), # count the number of times they've worked together
    mean_success_score = mean(success_score, na.rm = TRUE)) |> # find the mean success score
  filter(mean_success_score > 0.60, success_count > 3) |>
  arrange(desc(mean_success_score), desc(success_count)) |>
  head(10)
```
Peter Jackson is the director of the Hobit. We can see how directors of franchises and TV series may skew the success count.
...
# Find a classic movie to remake with your key talent. 
# The original should have a large number of IMDb ratings, a high average rating, and not have been remade in the past 25 years.4

Once you have found your classic movie to remake, confirm whether key actors, directors, or writers from the original are still alive. 
You may also want to include the classic actors as “fan service.”


## Elevator Pitch
“The market for animated young adult horror musicals has grown 200% in the past decade” or “Over 90% of Director D’s movies are successes.”

Strong Box Office Appeal: By casting Emma Stone and John Boyega, we leverage their proven track records; both actors have consistently delivered hits, with Emma starring in over 25 successful films and Boyega emerging as a fan favorite with a strong following.

Nostalgia Meets Modernity: Remaking The Princess Bride, a classic with an 8.1 IMDb rating and over 800,000 ratings, taps into a well-loved story while allowing us to introduce fresh talent, including original cast cameos, to engage both longtime fans and new audiences.

Proven Directorial Vision: With Greta Gerwig at the helm, whose films average an IMDb rating of 8.2, we ensure a balance of heartfelt storytelling and commercial success, making this project a compelling addition to the current film landscape.

## Brief Trailer
From director Greta Gerwig, the visionary mind behind Lady Bird; and from Emma Stone, beloved star of La La Land; and from John Boyega, Hollywood icon of the sci-fi genre, comes the timeless tale The Princess Bride. A story of love, adventure, and the power of friendship—coming soon to a theater near you.

Imagine a heartwarming remake of The Princess Bride, the beloved classic that combines romance, adventure, and humor. With Emma Stone and John Boyega as our lead duo, we tap into their proven track record of box office success and audience appeal—Emma has starred in over 25 successful films, while John brings a fresh energy that resonates with younger viewers. Directed by Greta Gerwig, whose films boast an average IMDb rating of 8.2, we are poised to create a timeless story that honors the original while captivating a new generation.
