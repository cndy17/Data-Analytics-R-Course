...
calculate_portfolio_value <- function(start_salary, years_worked, retirement_age, current_age) {
  ...
  for (age in current_age:(retirement_age - 1)) {
    ...
    current_salary <- current_salary * (1 + long_run_averages$wage_growth_avg)
  }
...
# TRS Simulation
simulate_trs_monte_carlo <- function(n_simulations, starting_salary, years_worked, wage_growth_data, inflation_data) {
  wage_growth_data <- na.omit(wage_growth_data)
  inflation_data <- na.omit(inflation_data)
  ...
  for (sim in 1:n_simulations) {
    ...
    for (i in 1:years_worked) {
      # Resample wage growth and inflation data for each year
      growth_rate <- sample(wage_growth_data$wage_growth_percent, 1)  
      inflation_rate <- sample(inflation_data$inflation_rate, 1) 
      salary <- salary * (1 + growth_rate + inflation_rate)
      salaries[i] <- salary
    }
         cat("Simulation:", sim, "Year:", i, "\n")
      cat("Wage Growth Rate: ", growth_rate, "\n")  # Print in percentage
      cat("Inflation Rate: ", inflation_rate, "\n")  # Print in percentage
      
    # Calculate final average salary (last 3 years)
    final_average_salary <- mean(tail(salaries, 3))
    
    # Calculate pension based on years worked
    pension <- retirement_benefit(years_worked, final_average_salary)
    monthly_pension <- pension / 12
    results[sim] <- pension
    # results[sim] <- monthly_pension
  }
  return(results)
}
...
#| code-title: View Code
# ORP Simulation
simulate_orp <- function(n_simulations, starting_salary, years_worked, 
  ...
  for (i in 1:n_simulations) {
    ...
    for (age in starting_age:(retirement_age - 1)) {
      ...
      wage_growth <- sample(wage_growth_data$wage_growth_percent, 1)
      inflation_rate <- sample(inflation_data$inflation_rate, 1)
      ...
      bonds_return <- (bond_return/100)
      short_term_return <- (debt_return/100)
      ...
      portfolio_value <- portfolio_value * (1 + us_equities_pct * equity_return_SPY +
                                            ...
                                            bonds_pct * bonds_return +
                                            short_term_pct * short_term_return)
      current_salary <- current_salary * (1 + wage_growth)
      year <- year + 1
    }
...
orp_results <- simulate_orp(n_simulations, starting_salary, years_worked, 
                           WAGE, INFLATION, SPY, ACWI, BOND, DEBT)
# Calculate the probability that an ORP employee exhausts their savings before death

# Probability that ORP > TRS
orp_vs_trs_prob <- mean(results$orp_results > results$trs_results)

# Report the results
cat("Probability ORP exhausts savings:", round(orp_exhaustion_prob, 4), "\n")
cat("Probability ORP has higher income than TRS:", round(orp_vs_trs_prob, 4), "\n")
