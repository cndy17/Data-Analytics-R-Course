...
The ORP is a **defined-contribution plan**, similar to a 401(k). In this plan, both you and CUNY make contributions to your individual retirement account, and the amount you have at retirement depends on several factors:

- **Market Risk**: Your retirement benefit is subject to the market performance of the investments you choose, so the value of your account can fluctuate over time.
- **Flexible Contributions**: You and CUNY will contribute a percentage of your salary into the plan. Typically, the employer contributes 8-10% depending on your years of service, while you contribute a portion as well.
- **Investment Choices**: The ORP gives you more control over your investment choices, often offering mutual funds with varying levels of risk (equities, bonds, etc.).

Your ORP retirement benefit depends on:
...
- **Investment Returns**: The money is invested in mutual funds, and your retirement account grows based on the returns of those investments.
Your return will depend on both the contributions and the market returns over your career. Given that the stock market can be volatile, your final portfolio value may fluctuate.
...
# Historical Data
We will understand the two retirement plan options by leveraging historical data from FRED and Alpha Vantage. Specifically, we'll focus on key metrics, such as wage growth, inflation rates, equity returns (from U.S. and international markets), and bond returns. 
...
**Bonds and Debt Returns**:
Bonds Returns: Bonds are typically considered a safer, less volatile investment compared to equities. They can provide steady income and help balance the risk of a retirement portfolio.
...
A key part of portfolio management is diversification, or spreading investments across different asset classes to reduce risk. Understanding how different assets (e.g., equities, bonds, inflation, and wage growth) are correlated is critical in this process.
...
First, I will be using long run averages for the two retirement plans. 
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
...
```
**Contribution Based on Last 3 Years of Salary**
The TRS pension benefit is based on your final average salary (FAS), which is determined by the average salary over the last three years of employment.
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
...
The first month retirement benefit for TRS is calculated by dividing the total annual pension by 12 months.
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
...
**Contribution Rate**
The employee's contribution is based on their salary, and the employer contributes a fixed percentage based on years of service. We will be following the Fidelity Freedom Fund distribution of funds for your contributions. The allocation of funds between equities (both domestic and international), bonds, and short-term debt changes as you age.
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
...
**Calculate for ORP**
This function simulates the growth of your ORP account based on contributions, salary increases, and asset allocation.
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
...
Here we compare the first month's retirement benefit from both plans.
**TRS First Month Retirement Benefit**
The first month retirement benefit for TRS is calculated by dividing the total annual pension by 12 months.
**ORP First Month Retirement Benefit**
After retirement, the ORP monthly benefit can be calculated based on a 4% withdrawal rate, which is typical for sustainable withdrawals.
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
...
The TRS pension offers a steady, inflation-adjusted income during retirement. This income increases each year based on inflation, but the increase is capped at 3% annually. The adjustment is effective every September, using the Consumer Price Index (CPI) of the previous 12 months to calculate the inflation rate.
**Inflation Adjustement**
To simulate the TRS benefit after retirement, we adjust the pension payments based on inflation. The adjustment is capped at 3% annually. The inflation adjustement is effective each September and the CPI used is the aggregate monthly CPI of the previous 12 months.
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
...
With the ORP, the employee builds a retirement portfolio over their working years and begins drawing down funds after retirement. The amount withdrawn each year is typically a percentage of the portfolio value, with a common recommendation being 4% annually.
In addition to withdrawals, the remaining portfolio continues to grow during retirement at an assumed annual return of 3%. This growth helps maintain the portfolio balance and ensures that funds last throughout retirement. 
The following code simulates the withdrawals from the ORP portfolio over a 20-year retirement period. The initial portfolio value is based on the employee's salary, years of service, and contributions. The code assumes an annual withdrawal of 4% and a 3% annual growth rate.
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
...
Since ORP is withdrawn annually, to compare it with the monthly TRS withdrawals, below, I am displaying ORP withdrawals on a month to month basis. 
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
...
The annual 4% withdrawals are counterbalanced by the assumed annual growth of the portfolio by 3%. 
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
# Check for remaining balance in ORP at the end of retirement
final_balance <- tail(withdrawals_data$Portfolio_Balance, 1)
if (final_balance < 0) {
  cat("The ORP runs out of funds in the year ", which(withdrawals_data$Portfolio_Balance < 0)[1], "\n")
} else {
  cat("The ORP has a final balance of $", round(final_balance, 2), "\n")
}
```
**Average monthly income/withdrawal (TRS vs ORP)**
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
...
The maximum gap represents the largest difference where TRS income was higher than ORP income.
The minimum gap represents the largest difference where ORP income was higher than TRS income (a negative value indicates ORP is higher).
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
...
combined_data <- tibble(
  Month = seq(1, min(nrow(trs_income_df), nrow(withdrawals_data))),
  TRS_Income = trs_income_df$Pension_Benefit[1:min(nrow(trs_income_df), nrow(withdrawals_data))],
  ORP_Income = withdrawals_data$Withdrawals[1:min(nrow(trs_income_df), nrow(withdrawals_data))]
)

# Reshape the data into a long format for ggplot
combined_data_long <- combined_data |>
  pivot_longer(cols = c("TRS_Income", "ORP_Income"), 
               names_to = "Plan", 
               values_to = "Income") |>
  mutate(Income = ifelse(Plan == "ORP_Income", -Income, Income))  # Reverse ORP income to match increasing trend

# Create the plot
ggplot(combined_data_long, aes(x = Month, y = Income, color = Plan)) +
  geom_line(size = 1) +
  labs(title = "Comparison of TRS and ORP Withdrawals/Income Over Retirement",
       x = "Years After Retirement", 
       y = "Monthly Income ($)",
       color = "Retirement Plan") +
  theme_minimal() +
  scale_color_manual(values = c("TRS_Income" = "blue", "ORP_Income" = "red")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
## TRS Simulation
Key Assumptions:
- Salary growth: Simulated using historical wage growth data.
- Inflation: Applied annually to adjust pension payments.
- Inflation-adjusted benefits: We simulate TRS benefits with a 3% maximum annual inflation increase.
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: View Code
...
ggplot(trs_results_df, aes(x = trs_benefit)) +
  ...
  geom_vline(aes(xintercept = mean(trs_benefit) + sd(trs_benefit)), color = "blue", linetype = "dotted", size = 1.2) +
  geom_text(aes(x = mean(trs_benefit), y = 0.2, label = paste("Mean:", round(mean(trs_benefit)))), 
            color = "black", vjust = -40, size = 3) +
  geom_text(aes(x = mean(trs_benefit) - sd(trs_benefit), y = 0.2, 
                label = paste("- 1 SD:", round(mean(trs_benefit) - sd(trs_benefit)))), 
            color = "black", vjust = -40, size = 3) +
  geom_text(aes(x = mean(trs_benefit) + sd(trs_benefit), y = 0.2, 
                label = paste("+ 1 SD:", round(mean(trs_benefit) + sd(trs_benefit)))), 
            color = "black", vjust = -40, size = 3) +
...
Key Assumptions:
- Employee and Employer Contributions: Contributions are based on salary and a specified contribution rate.
- Asset Allocation: The portfolio grows based on a mix of equities, bonds, and other assets. This mix changes based on the employee's age.
- Investment Returns: Simulated using historical returns from a variety of asset classes.
...
simulate_trs_monthly <- function(n_simulations, initial_salary, years_worked, wage_growth_data, inflation_data) {
  wage_growth_data <- na.omit(wage_growth_data)
  inflation_data <- na.omit(inflation_data)
  ...
  for (sim in 1:n_simulations) {
    ...
    for (i in 1:years_worked) {
      growth_rate <- sample(wage_growth_data$wage_growth_percent, 1)  # Sample growth rate from the data
      inflation_rate <- sample(inflation_data$inflation_rate, 1)  # Sample inflation rate
      salary <- salary * (1 + growth_rate + inflation_rate)  
      salaries[i] <- salary  
    }
    ...
    # Calculate TRS benefit (monthly payout) based on the FAS
    trs_benefit <- retirement_benefit_function(years_worked, final_average_salary)
    current_pension <- trs_benefit
    retirement_benefit[1] <- current_pension
    
    # Simulate monthly adjustments (based on inflation every September)
    for (current_month in 2:(years_worked * 12)) {
      current_year <- ceiling(current_month / 12) 
      current_month_in_year <- current_month %% 12
      if (current_month_in_year == 0) current_month_in_year <- 12  
...
monthly_trs_results <- simulate_trs_monte_carlo(n_simulations, starting_salary, years_worked, WAGE, INFLATION)

months <- rep(1:(years_worked * 12), n_simulations)  # Create a vector for the month numbers (1 to years_worked * 12)
simulations <- rep(1:n_simulations, each = years_worked * 12)  # Create a vector for simulation numbers (1 to n_simulations)
...
**Longevity Risk**: Given that the simulations show no ORP depletion, your funds should last throughout retirement. However, as a defined-contribution plan, the ORP can be more sensitive to market conditions. If you are risk-averse or expect to live longer than average, the stability of the TRS pension may appeal more to you.

**Investment Strategy**: Your ORP balance’s performance depends significantly on the investments you choose. If you're comfortable with market volatility and are willing to take an active role in managing your investments, the ORP can be a more rewarding option. On the other hand, the TRS offers more predictable income with no exposure to the ups and downs of the market.

**Retirement Goals**: If your goal is to have predictable, stable income for your entire retirement, the TRS plan’s defined benefit structure will provide more peace of mind. However, if you’re looking for greater control over your retirement funds and the potential for higher returns, the ORP might be a better fit.
