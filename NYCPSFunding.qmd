...
Within this overarching question, I will explore the following sub-questions surrounding funding and spending:
...
sample_n(REGENTS, 100) |>
  DT::datatable()
```

In the sample Regents data above, we can see that in row 2 (P.S. 019 Judith K. Weiss in 2015), there are 's' values. To clean the data from Info Hub, I am converting characters to the numeric data type, replacing 's' with NAs, and calculating averages for graduation rates.
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: Code
...
From the City of New York, I am loading the following data files on Math and English Langage Arts (ELA) scores:
...
Here I've created a function to pull exam result data from the City of New York. I use the function to download data for ELA scores for 2006-2012 and 2013-2019, ELA scores with Economic status for 2013-2017, ELA scores on a district level for 2006-2011 and 2013-2018. 

**ELA Score Data for 2006-12 (All Students)**
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: Load Data
#| code-summary: "View Code for 2006-2019 ELA Data Download"
...
head(ELA_2006_12_ALL)
```

Here, I define a function for ELA scores from 2013-2023 and download the data. 

**ELA Score Data for 2013-23 (All Students)**
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: Load Data
#| code-summary: "View Code for 2013-2023 ELA Data Download"
...
head(ELA_2013_23_ALL)
```

Next, I'm renaming the columns of `ELA_2006_2012_ALL` so that the column names are consistent across the ELA score data sets. This will help with binding the datasets and my analysis later on. Additionally, I am ensuring that columns such as the number, percent, and mean are numeric instead of characters. This will introduce NA where the value is 's'. After this, I can bind the ELA 2006-2012 and 2013-2023 data sets together.
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: CleanData
#| code-summary: "View Code for Cleaning ELA Data"
#| layout-ncols: 2
# Rename columns
...
**ELA Score Data for 2006-2023 (All Students)**
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: Bind Data
#| code-summary: "View Code for Binding ELA Data"
ELA_2006_23_ALL <- bind_rows(
  ELA_2006_12_ALL,
  ELA_2013_23_ALL)

sample_n(ELA_2006_23_ALL, 100) |>
  DT::datatable()
```

Next, I am loading in the Math 2006-2012, Math 2013-2023, and district level Math 2013-2023 data below. To clean the math data, like with the ELA data, I am renaming the columns to ensure consistency across the data sets. I am also binding the Math 2006-2012 and 2013-2023 data sets together and converting to the appropriate data types.

**Math Score Data for 2006-2023 (All Students)**

```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: Load  Data
#| code-summary: "View Code for Loading and Cleaning Math Data"
...
district_MATH_1323<- get_math1323_file("03_district-math-results-2013-2023-(public).xlsx", MATH_2013_23, "All", MATH_URL_2013_23)
 ...
 # add column to match 2013-23 df
...
#| code-fold: true
#| code-title: Code
#| code-summary: "View Code for Calculating Averages"
...
Below we can take a look at the distribution of the data we just downloaded and cleaned. 
...
I am downloading the School Funding Transparency Data for 2017-2019 and 2019-2023 from the NYC InfoHub site. I will use this data to analyze the funding distribution across schools in NYC. The datasets I will be using are:

- `funding_C_201823`

**School Funding Transparency Data for 2017-2019**
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: Code
#| code-summary: "View Code for Downloading School Funding Transparency Data"
...
sample_n(funding_C_201823, 1000) |>
  DT::datatable()
```
...
#| code-fold: true
#| code-title: Code
#| code-summary: "View Code for Charter School Data"
...
ELA and Math scores for Charter Schools is available through the City of New York site. I will download the data and clean it up using the functions defined previously. To clean the Charter School data, I will rename the columns to ensure consistency across the data sets. I will also convert the columns to the appropriate data types as we did with the other data sets. 
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: Code
#| code-summary: "View Code for Charter School Score Data"
# Load Charter School Data from City of New York
...
**Charter School Math and ELA Score Data for 2006-2023**
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: Code
...
sample_n(All_CS_combined, 100) |>
  DT::datatable()
```

## School Expenditures
The NYC Public School site publishes School Based Expenditure Reports for the years 2017 and 2018. I will scrape the data from the site and combine the two school years into one dataset. 

- `expenditure_201618`

**School Expenditure Data for 2017 and 2018 (2016-2018 School Year)**
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: Code
...
sample_n(expenditure_201618, 76) |>
  DT::datatable()
```
...
**District Spending Data for 2004-2012**
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: Code
#| code-summary: "View Code for District Specific Data"
...
sample_n(district_spending_0412, 100) |>
  DT::datatable()
```
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
...
#| code-summary: "View Code for Ridge Plot"
# order by district `csd`
district_spending_0412 <- district_spending_0412 |>
  arrange(csd)

ggplot(district_spending_0412, aes(x = grand_total_csd, y = as.factor(boro), fill = as.factor(boro))) +
  geom_density_ridges() +
  theme_ridges() + 
  labs(title = 'Total Spending per District', y = 'boro', x = 'Total Spending ($)') +
...
#| layout-ncol: 2
#| layout-spacing: normal
#| code-summary: "View Code for Summary Statistics for Academic Performance Metrics"
...
- Lower student achievement (incoming test scores)
...
- Special education periods mandated per week.

Students in Temporary Housing

- Self-reported living situation on the Housing Questionnaire.

Additional High School Weights: 
...
- Specialized Audition: audition or portfolio review, Arts Endorsed Diploma

- Transfer: re-engage drop out or over-age/under-credit students
...
#| code-fold: true
#| code-title: Code
#| code-summary: "View Code for List of District 75 Schools"

library(rvest)
library(dplyr)

# URL of the page to scrape
url <- "https://www.newyorkschools.com/districts/nyc-district-75.html"

# Read the HTML of the webpage
webpage <- read_html(url)

# Extract school names from anchor tags in the .pp-col-40 class
school_names <- webpage |>
  html_nodes(".pp-col-40 a") |>
  html_text(trim = TRUE)

# Clean up by removing any unwanted header (e.g., "School Name")
school_names <- school_names[!grepl("School Name", school_names)]
school_names <-as.data.frame(school_names)

# Print the list of school names
sample_n(school_names, 57) |>
  DT::datatable()
```

**Schools With the Least Funding**

We see that Young Adult Borough Centers (YABC) schools are receiving the least funding. YABC schols are typically part time, afternoon, and night programs for high schoolers who are behind in credits or cannot attend school during the day. All YABCs are managed by District 79. I would assume that these schools have less funding because they are *part time* in nature. YABC schools are also supplemented by the Learning to Work program. [^12] 

[^12]: https://www.schools.nyc.gov/enrollment/other-ways-to-graduate/young-adult-borough-centers
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| code-title: Code
#| code-summary: "View Code for Schools with the Least Funding"
avg_funding_per_school <- funding_C_201823 |>
  group_by(`School Name`) |>
  summarize(`Average Funding` = mean(`Total School Funding per Pupil`, na.rm = TRUE))|>
  arrange((`Average Funding`)) 

lowest_funding_table <- avg_funding_per_school |>
  head(10) |>  
  gt() |>  
  tab_header(
    title = "Bottom 10 Schools with Lowest Average Total Funding per Pupil"
  ) |> 
  cols_label(
    `School Name` = "School Name",
    `Average Funding` = "Average Funding per Pupil"
...
In addition to Charter Schools, New York City has eight specialized high schools. One of which is The Bronx High School of Science that focuses on science, technology, engineering, and mathematics (STEM) education.
...
- Admission: Based on the Specialized High Schools Admissions Test (SHSAT) score

Below we can see the funding amounts of these specialized high schools in 2018 and 2019. The median of funding across all schools is 26048.28, whereas the median funding across the specialized high schools is 17248.25.
```{r, echo = TRUE, message = FALSE, warning = FALSE} 
#| code-fold: true 
#| code-title: Code
#| code-summary: "View Code for Comparison of Specialized HS Funding"

# filter for DBN of specialised high schools
specialized_hs_funding <- funding_C_201823 |>
  filter(DBN %in% c("X445", "K449", "K430", "M485", " M692", "X696", "Q687", "R605", "M475"))

ggplot(specialized_hs_funding, aes(x = `DBN`, y = `Total School Funding per Pupil`, fill = factor(Year))) +  
  geom_col(position = "dodge", color = "white", size = 1) +  
  labs(
    title = "Total School Funding per Pupil for Specialized High Schools", 
    x = "School",
    y = "Total School Funding per Pupil"
  ) +
...
# see if schools are above or below average funding for overall schools
overall_median_funding <- median(funding_C_201823$`Total School Funding per Pupil`, na.rm = TRUE)
shs_median_funding <- median(specialized_hs_funding$`Total School Funding per Pupil`, na.rm = TRUE)

total_schools <- nrow(specialized_hs_funding)
above_median_count <- sum(specialized_hs_funding$Above_Median, na.rm = TRUE)
percentage_above_median <- (above_median_count / total_schools) * 100
median_values <- data.frame(
  Statistic = c("Overall Median Funding", "Specialized HS Median Funding", "Specialized HS with Funding Above Overall Median Funding"),
  Value = c(paste0("$",round(overall_median_funding, 2)), paste0("$", round(shs_median_funding, 2)), paste0(round(percentage_above_median, 2), "%"))
)
...
median_values |>
  ...
  tab_header(
    title = "Comparison of Median Funding and Percentage of Schools Above Median"
  ) |>
  fmt_number(
    columns = vars(Value),
    decimals = 2  
...
Despite a lower median funding, specialized high schools maintain a higher level of academic performance. The specialized high schools have a higher mean score on the Regents exams compared to non-specialized high schools across the regents exams.
```{r, echo = TRUE, message = FALSE, warning = FALSE} 
#| code-fold: true 
#| code-title: Code
#| code-summary: "View Code for Specialized HS Regents Scores"

# note: the specialized high schools have different school codes in the REGENTS df than the funding df
# Create a new column for School Type (Specialized vs Non-Specialized)
SHS_REGENTS <- REGENTS |>
  mutate(School_Type = ifelse(DBN %in% c("10X445", "14K449", "13K430", "03M485", "05M692", "10X696", "28Q687", "31R605", "02M475"),
                              "Specialized High Schools", "Non-Specialized High Schools")) |>
    filter(!is.na(`Mean Score`))

REGENTS_mean <- SHS_REGENTS |>
  group_by(`Regents Exam`, School_Type) |>
  summarise(Mean_Score = mean(`Mean Score`, na.rm = TRUE))


# plot the regents scores and compare with non specialized high school
ggplot(REGENTS_mean, aes(x = School_Type, y = `Mean_Score`, fill = School_Type)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = round(`Mean_Score`, 1)), 
            ...
            size = 3) +
  labs(
    title = "Comparison of Mean Regents Scores by School Type",
    x = "School Type",
    y = "Mean Score"
  ) +
  ...
  facet_wrap(~ `Regents Exam`, scales = "fixed") +
  ylim(0, 100)
```
...
#| code-fold: true
#| code-title: Code
#| code-summary: "View Code for Spending Map"
...
spending_map_df_0412 <- nyc_sf |>
  left_join(district_spending_0412, by = c("SchoolDist" = "csd")) 

grand_total_df <- spending_map_df_0412[, c("year", "SchoolDist", "grand_total_csd", "grand_total_boro", "grand_total_nyc")]

spending_map_df_0412$grand_total_nyc <- as.numeric(spending_map_df_0412$grand_total_nyc)

spending_map_df_0412_gif <- spending_map_df_0412 |> 
  ggplot() + 
  ...
  labs(title = "Spending by District in NYC (Total) - Year: {frame_time}")

# anim_save("spending_map_0412.gif", animation = spending_map_df_0412_gif)
```
![](images/spending_map_0412.gif)
