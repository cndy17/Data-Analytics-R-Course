...
First, we need to clean the data to ensure they join properly. First, we convert to the same CRS. Then, I am adding a `STATENAME` column based on the `STATEFP.`  
``` {r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| title: View Code
...
create_election_data <- function(election_year, shapefile_data) {
  ...
  election_data <- PRESIDENT |>
    ...
    filter(party_simplified %in% c("DEMOCRAT", "REPUBLICAN")) |>
    group_by(state, state_fips, party_simplified, year) |>  # Group by state and party
  ...
  dis_election <- 
    left_join(shapefile_data, election_data, by c("STATENAME" = "state"), relationship = "many-to-many")
  return(dis_election)
}
# create and bind data for each election
...
#| code-fold: true
#| title: View Code
create_election_data <- function(election_year) {
#Filter for the specific year and the simplified party
  election_data <- PRESIDENT |>
    filter(year == election_year, office == "US PRESIDENT") |>  # Filter for the specific year and presidential election
    filter(party_simplified %in% c("DEMOCRAT", "REPUBLICAN")) |>
    group_by(state, state_fips, party_simplified, year) |>  # Group by state and party
...
all_election_data <- bind_rows(
  election_data_2020 <- create_election_data(2020),
  election_data_2016 <- create_election_data(2016),
  election_data_2012 <- create_election_data(2012),
  election_data_2008 <- create_election_data(2008),
  election_data_2004 <- create_election_data(2004),
  election_data_2000 <- create_election_data(2000),
  election_data_1996 <- create_election_data(1996),
  election_data_1992 <- create_election_data(1992),
  election_data_1988 <- create_election_data(1988),
  election_data_1984 <- create_election_data(1984),
  election_data_1980 <- create_election_data(1980),
  election_data_1976 <- create_election_data(1976)
)
```
...
#| code-fold: true
#| title: View Code
# dowload state shapefiles
get_state_file <- function(fname, year){
  BASE_URL <- sprintf("https://www2.census.gov/geo/tiger/TIGER%d/STATE/", year) #replace %d with year
  fname_ext <- paste0(fname, ".zip")
  fname_ext1 <- paste0(fname, ".shp")
  fname_extunzip <- gsub(".zip$", "", fname_ext)
...
tl_2018_us_state <- get_state_file("tl_2018_us_state", 2018)

#Join with the shapefile data
shape_election <- left_join(tl_2018_us_state, all_election_data, by = c("NAME" = "state"), relationship = "many-to-many") |>
  filter(!is.na(year))
```
...
all_election_maps <- ggplot(shape_election, aes(geometry = geometry, fill = winning_party)) +
  geom_sf() +
  scale_fill_manual(values = c("DEMOCRAT" = "blue", "REPUBLICAN" = "red")) +
  theme_minimal() +
  labs(title = "Presidential Election State Results {closest_state}",
...
save_directory <- here()  # Automatically get the directory of the script
animate(all_election_maps, renderer = gifski_renderer(file = paste0(save_directory, "/election_results_animation.gif")))
```
...
We can find the electoral college votes using the `House` data. 
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| title: View Code
...
This method follows that the candidate who wins the most votes in a state receives all of that state’s Electoral College votes
...
The approach here invovles calculating the total number of votes for each candidate in each state. Then, determine the proportion of the total vote that each candidate received in each state.
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#| code-fold: true
#| title: View Code
